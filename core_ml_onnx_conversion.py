"""
This script is used to convert the trained model to ONNX format, to be later converted to CoreML format.
"""
import torch
...
def convert_to_coreml(model, args, device):
    ...
    # )
    torch_onnx_export(model, img, 'espnetv2.onnx', export_params=True)
    print_info_message('ONNX model saved successfully in {}'.format(args.savedir))
    # try:
    #     espnetv2_onnx_model
