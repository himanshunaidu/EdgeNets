...
def main(args):
    ...
    if args.dataset == 'city':
        ...
        val_dataset = CityscapesSegmentation(root=args.data_path, train=False, size=(args.im_size[0], args.im_size[1]), 
                                             scale=args.scale,
                                             coarse=False)
    ...
    miou_val, val_loss = val(model, val_loader, None, seg_classes, device=device)
    print_info_message('Validation mIoU: {:.2f}'.format(miou_val))
    print_info_message('Validation loss: {:.4f}'.format(val_loss))
...
if __name__ == '__main__':
    ...
    if args.dataset == 'pascal':
        ...
        if args.im_size[0] == 512:
            args.scale = (0.25, 0.5)
        elif args.im_size[0] == 1024:
            args.scale = (0.35, 1.0)  # 0.75 # 0.5 -- 59+
        elif args.im_size[0] == 2048:
            args.scale = (1.0, 2.0)
        else:
